...
type TagQuery struct {
	...
	withServers *FavoriteServerQuery
...
func (tq *TagQuery) QueryServers() *FavoriteServerQuery {
	query := (&FavoriteServerClient{config: tq.config}).Query()
...
func (tq *TagQuery) WithServers(opts ...func(*FavoriteServerQuery)) *TagQuery {
	query := (&FavoriteServerClient{config: tq.config}).Query()
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withServers; query != nil {
		if err := tq.loadServers(ctx, query, nodes,
			func(n *Tag) { n.Edges.Servers = []*FavoriteServer{} },
			func(n *Tag, e *FavoriteServer) { n.Edges.Servers = append(n.Edges.Servers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadServers(ctx context.Context, query *FavoriteServerQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *FavoriteServer)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.ServersTable)
		s.Join(joinT).On(s.C(favoriteserver.FieldID), joinT.C(tag.ServersPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.ServersPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ServersPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*FavoriteServer](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
